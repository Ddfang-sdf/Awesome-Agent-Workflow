(() => {
  ...
  const nodeDetails = {
    ...
    "task-split": {
      ...
      type: "EXECUTION PLAN",
      title: "有序任务拆分",
      summary: "把通过门禁的设计切成小而完整、可独立验收的开发单元。",
      input: "正式详细设计与测试用例设计，两者都必须已通过门禁。",
      action: "按依赖关系生成任务总览与 T1、T2…任务卡，每张任务卡都包含目标、范围和完成定义。",
      output: "<模块组>_tasks/overview.md，以及至少一个 T<N>-<任务标题>.md。",
      release: "用户必须确认任务计划；随后按任务列表生成 task-dev，调度方式固定为串行。",
      exception: "只有 overview 没有任务卡、任务列表为空或编号被重复写入时，拒绝推进并补齐任务文件。"
    },
    "task-dev": {
      index: "13",
      ...
      input: "任务总览、当前任务卡、软件架构；规格与编码规范存在时一并继承。",
