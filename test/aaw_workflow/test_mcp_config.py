...
def posix(p) -> str:
    ...
    """Config files use forward slashes uniformly."""
...
class TestUpsertClaudeMcp(unittest.TestCase):
    ...
    def test_it_m04_03_path_update(self) -> None:
        ...
        assert data["mcpServers"]["question-tracker"]["command"] == posix(exe_new)
    ...
    def test_it_m04_05_project_scope(self) -> None:
        ...
        assert proj_mcp["command"] == posix(exe)
    ...
    # -- IT-M04-08 ~ IT-M04-11: JSONC tolerance (comments in config) -------



    def test_it_m04_08_jsonc_line_comments_preserved(self) -> None:

        """IT-M04-08: claude.json with // line comments — user content must survive."""
        ...
        config.write_text(
            ...
            '    "my-other-server": { "command": "/usr/bin/other" }\n'
        ...
        assert data["projects"]["/some/proj"]["allowedTools"] == ["bash"]

        assert data["mcpServers"]["my-other-server"]["command"] == "/usr/bin/other"

        assert data["mcpServers"]["question-tracker"]["command"] == posix(exe)



    def test_it_m04_09_jsonc_block_comments_preserved(self) -> None:

        """IT-M04-09: claude.json with /* */ block comments — content must survive."""
        ...
        config.write_text(
            ...
            '  "mcpServers": { "existing": { "command": "/bin/x" } }\n'
        ...
        assert data["mcpServers"]["existing"]["command"] == "/bin/x"

        assert data["mcpServers"]["question-tracker"]["command"] == posix(exe)



    def test_it_m04_10_comment_inside_string_not_stripped(self) -> None:

        """IT-M04-10: // inside a string value (e.g. URL) must NOT be treated as comment."""
        ...
        config.write_text(
            ...
            '  "homepage": "https://example.com/docs",  // homepage\n'
        ...
        assert data["homepage"] == "https://example.com/docs"

        assert data["theme"] == "dark"



    def test_it_m04_11_unparseable_config_refused_not_wiped(self) -> None:

        """IT-M04-11: totally broken JSON → refuse to write, file unchanged."""

        exe = self._touch_exe()

        config = self.tmp_path / "claude.json"

        broken = '{ "mcpServers": { invalid json ,,,'
...
class TestUpsertCodexMcp(unittest.TestCase):
    ...
    def test_it_m05_01_new_file(self) -> None:
        ...
        assert f'command = "{posix(self.exe)}"' in content
    ...
    def test_it_m05_05_path_update(self) -> None:
        ...
        assert posix(self.exe) in content
...
class TestUpsertOpencodeMcp(unittest.TestCase):
    ...
    def test_it_m06_01_new_file(self) -> None:
        ...
        assert mcp["command"] == [posix(self.exe)]
    ...
    def test_it_m06_03_legacy_migration(self) -> None:
        ...
        assert data["mcp"]["question-tracker"]["command"] == [posix(self.exe)]
    ...
    def test_it_m06_06_jsonc_comments_preserved(self) -> None:

        """IT-M06-06: opencode.json with // comments — user content must survive."""
        ...
        config.write_text(
            ...
            '    "my-server": { "type": "local", "command": ["/usr/bin/my"] }\n'
        ...
        assert data["mcp"]["my-server"]["command"] == ["/usr/bin/my"]

        assert data["mcp"]["question-tracker"]["command"] == [posix(self.exe)]



    def test_it_m06_07_unparseable_refused_not_wiped(self) -> None:

        """IT-M06-07: broken opencode.json → refuse to write, file unchanged."""
...
class TestEnsureMcpConfig(unittest.TestCase):
    ...
    def test_it_m08_01_chrys_full_flow(self) -> None:
        ...
        if code_yaml.is_file():
            ...
            content = code_yaml.read_text(encoding='utf-8')
    ...
    def test_it_m08_07_opencode_full_flow(self) -> None:
        ...
        assert data["mcp"]["question-tracker"]["command"] == [posix(exe)]
