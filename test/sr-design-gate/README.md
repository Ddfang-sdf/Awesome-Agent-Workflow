...
通过样本必须在没有历史报告时给出零问题 `pass`，并生成门禁报告。
...
4. 对通过样本，确认结论为 `pass`，6 个 summary 计数均为 0，生成门禁报告，且
   “图表变更表达”维度达标。

## 通过标准

- `conflicts`：结论必须为 `fail`，10 个预置冲突均被定位并给出整改要求；不得放行。
- `pass`：结论必须为 `pass`，六个 summary 计数均为 0，且生成门禁报告并在 `report` 中填写其路径。
- 两个样本中的系统方案图均须使用固定色板和图内图例；首次 AR 拆分的依赖图按例外规则
  不强行标记系统变化状态。
