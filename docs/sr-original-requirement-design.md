...
6. 目标文件已存在时的处理：
   ...
   - 已存在内容**不一致**：启动失败并提示用户显式处理（确认保留旧文件，或手动删除后重启），
     不得静默覆盖；
...
开头若干行并注明总行数），请用户确认与其提供的原始需求一致。用户指出不一致时，按
5.1 规则 6 的不一致分支处理（删除后以正确内容重启）。
...
不新增强制门禁报告。仍沿用当前按需报告规则：首次零问题通过时可以不生成
`SR-design-gate.md`；发现遗漏时必须生成报告并指出原始需求原文及缺失位置。
...
- 用户在设计中途提出真实的原始需求变更时，Agent 不自行改写来源文件，而是提示用户，
  由**用户本人**更新 `original-requirement.md`（或提供新的需求文件重新走 5.1 的不一致
  分支处理）；
