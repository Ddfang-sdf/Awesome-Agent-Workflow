...
本模板记录 ASIS 查证的现状事实、证据索引、规格漂移和待确认项，供 TOBE 和 Gate 引用。所有会影响 TOBE、AICoding、验证或风险判断的 ASIS 结论，都必须在 `.context.md` 中有稳定编号和证据编号。
...
生成或更新 `.context.md` 时，必须保留本模板的固定目录 `C1` 至 `C8`。某章暂时没有内容时，不得删除标题，应写：
