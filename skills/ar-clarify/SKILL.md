...
2. 调用 `list_sessions` 检查该池是否存在：
   - **不存在** → 不得继续裸跑：进入 5.1 立即建池，首个决策点入池即完成建池；`add_questions` 未成功返回前不得向用户提问。
...
分析 SR 文档中该 AR 的描述和代码现状，找出最顶层的实现决策点——通常是"这个 AR 的核心实现方案是什么"或"接口/数据的改动范围在哪里"。调用 `add_questions`（`session: <池名>`）将这个决策点加入问题池，然后立即进入 5.2 的展示流程。此调用同时承担建池职责：若返回错误，按上方「MCP 错误处理」表处置并重试，确认成功前不得进入 5.2。
