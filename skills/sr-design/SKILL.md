...
- 原始需求（`original-requirement.md`）中的每项明确需求在设计正文、AR 拆分和需求追溯章节中有落点
- 经用户确认为范围外、延期或不实现的原始需求，已在需求追溯表中显式记录一行（原始需求原文引用或可定位的原文位置、处置结论、用户确认来源如问题池编号）——只存在于对话记忆中、未落入文档的确认视为无确认，Gate 将判定为静默省略
- 原始需求与后续用户澄清存在冲突时，已向用户确认并以确认结果为准，未自行丢弃原始内容
- 用户明确提出的需求在文档中未体现
- 澄清过程中确认的细节在文档中未写入
- 模板中的占位符未替换
- 模板中的章节有缺失
