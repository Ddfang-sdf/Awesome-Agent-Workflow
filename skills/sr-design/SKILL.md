...
分析用户的需求描述，找出最顶层的设计决策点——通常是"这个需求的核心要解决什么问题"或"需求的范围边界在哪里"。调用 `add_questions`（`session: <池名>`）将这个决策点加入问题池，然后立即进入 5.2 的展示流程。此调用同时承担建池职责：若返回错误，按上方「MCP 错误处理」表处置并重试，确认成功前不得进入 5.2。
