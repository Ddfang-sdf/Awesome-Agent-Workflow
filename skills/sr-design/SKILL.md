...
- 以下章节承载设计主干，任何情况下都不得判定为不适用：对外接口、主成功场景、整体架构、AR 拆分与交互定义全章、需求追溯、SR 整体验收标准。这些章节内容缺失时必须停止工作，并回到澄清流程补齐，严禁对这些章节私自标注不适用。
...
- 变更影响分析要求：涉及既有行为变更时，逐项给出变更前与变更后的对比结果，并形成 E0-E3 分级结论。每项"无影响/不变"结论必须附可反查证据（设计章节、现有接口或代码位置、调用链检索结果、计划测试用例、估算依据）。禁止使用"改动行数小"、"仅内部重构"、"接口签名未变"、"单元测试通过"作为无对外影响的唯一证据。证据不足时标记"待确认"，不得直接形成确定分级。变更前状态引用现状分析章节的对应表格行，不重复描述。
